...
lon_0 <- 7.5
lon_1 <- 10
latitude <- c(lat_0,lat_1)
longtitude <- c(lon_0,lon_1)
time_step <-6
...
df_shifted_lon <- shift_col(df_temp,"longitude",1)
df_temp$shifted_lon <- df_shifted_lon$longitude
###Checks if two executive rows belong to the same longitude
df_temp$same_lat <- df_temp$longitude == df_temp$shifted_lon
...
df_false <- df_temp[df_temp$same_lat==TRUE, ]
df_true <- df_temp[df_temp$same_lat==FALSE, ]
### To substract two executive temperatures for interpolation function 
df_false$shifted_t <-  (shift_col(df_false,"temperature",1))$temperature
df_true$shifted_t <-  (shift_col(df_true,"temperature",1))$temperature
...
df_false$t_interp_lon <- (cte_long*df_false$shifted_t + (1-cte_long)*df_false$temperature)
df_true$t_interp_lon <- (cte_long*df_true$shifted_t + (1-cte_long)*df_true$temperature)
...
indices_lat_0 <- which(df_temp$latitude==lat_0)
indices_lon_0 <- which(df_temp$longitude==lon_0)
df_temp_lat <- df_temp[indices_lon_0,]
...
df_joined$temp_interp <- cte_lat*df_true_red$t_interp_lon+(1-cte_lat)*df_false_red$t_interp_lon
