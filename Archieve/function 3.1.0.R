...
retrieve_data <- function(latitude,longitude,months,years)
{
  ...
  names(north_it_df_t)[names(north_it_df_t) == "latitude"] <- "lat_temp_hum"
  ...
  names(north_italy_df_rad)[names(north_italy_df_rad) == "latitude"] <- "lat_set2" 
  names(north_italy_df_rad)[names(north_italy_df_rad) == "longitude"] <- "lon_set2"
  ...
  df_merge$lat_set2 <- north_italy_df_rad[,"lat_set2"]
  df_merge$lon_set2 <- north_italy_df_rad[,"lon_set2"]
...
df <- retrieve_data(latitude,longitude,months,years)
...
df_final <- std_date_time(df)
df_final <- interpolate(df_final,"temperature")
names(df_final)[names(df_final) == "feature_interp"] <- "temperature"
df_final$RH <- interpolate(df_final,"RH")$feature_interp
